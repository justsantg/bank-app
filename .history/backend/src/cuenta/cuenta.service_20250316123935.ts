...
export class CuentaService {
  ...
  async create(createCuentaDto: CreateCuentaDto): Promise<Cuenta> {
    ...
    // 🔥 Buscar el cliente antes de crear la cuenta
    const clienteExiste = await this.cuentaRepository.manager.findOne(Cliente, {
      where: { id: createCuentaDto.clienteId },
    });
    ...
    const cuenta = this.cuentaRepository.create({
      ...createCuentaDto,
      cliente: clienteExiste, // 🔥 Asignamos el objeto cliente
    });
